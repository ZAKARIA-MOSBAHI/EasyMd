...
import  {markdownService} from './Editor/lib/turndown/turndown-service.ts'; 
...
export default function Preview({value = initialContent} : {value? : string}) {
  const [md , setMd ] = useState<string>("");
useEffect(() => {
  const markdown = markdownService.turndown(value);
  setMd(markdown);
}, [value]);
  return (
    <div className="markdown_preview" style={{padding : '1rem' , whiteSpace : 'pre-wrap' , textAlign : 'left' , wordBreak :'break-word'}}>
      {md}
    </div>
  );
}
